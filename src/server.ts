		...
		for (const group of productGroups) {
			...
			for (let i = 0; i < productUrls.length; i += concurrency) {
				...
				results.forEach((result, idx) => {
					if ((result as PromiseFulfilledResult<any>).status === "fulfilled" && (result as PromiseFulfilledResult<any>).value.success) {
						addedCount++;
					} else {
						errors.push({ url: batch[idx], error: (result as any).reason?.message || "Unknown error" });
					}
				});
			}
		}
