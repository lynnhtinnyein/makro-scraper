...
app.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        ...
        const maxProducts = Math.min(parseInt(String(max)) || 5, 5);
...
app.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        if (activeBatchRequests >= MAX_CONCURRENT_BATCHES) {
            res.status(429).json({
                error: "Server is busy processing another batch request. Please try again later.",
                activeBatches: activeBatchRequests
            });
            return;
        }
...
const version = process.env.VERSION || "2.2.3";
