...
async function submitProduct(token, productData, categoryIds, productAttributeValueId) {
    ...
    const response = await axiosInstance.post("https://api.ecommerce.neon-xpress.com/v1/api/product/saveWithVariants", payload, { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } });
...
async function uploadProductImages(token, productId, imageUrls) {
    ...
    for (let i = 0; i < imagesToUpload.length; i += concurrency) {
        ...
        const promises = batch.map((imageUrl) => axiosInstance
            .post("https://api.ecommerce.neon-xpress.com/v1/api/product-image/save-image-url", { productId, imageUrl }, { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } })
            .catch((error) => {
