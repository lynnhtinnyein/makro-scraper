        ...
        for (const group of productGroups) {
            ...
            for (let i = 0; i < productUrls.length; i += concurrency) {
                ...
                const results = await Promise.allSettled(batch.map(async (productUrl) => {
                    ...
                    const productId = await (0, submit_1.submitProduct)(token, product, { mainCategoryId, subCategoryId, categoryId, sellerId }, productAttributeValueId);
                    if (productId && product.images?.length > 0) {
                        await (0, submit_1.uploadProductImages)(token, productId, product.images);
                    }
                    return { success: true };
                }));
